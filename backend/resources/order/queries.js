...
const setupOrderItems = "CREATE TABLE order_items (order_item_id SERIAL PRIMARY KEY, order_id INT NOT NULL, item_id INT NOT NULL,  quantity INT NOT NULL DEFAULT 1, item_price DECIMAL(10,2) NOT NULL, FOREIGN KEY (order_id) REFERENCES orders(order_id), FOREIGN KEY (item_id) REFERENCES items(item_id))";
...
const getOrderItems = "SELECT item_id, quantity, item_price from order_items WHERE order_id = $1";

export default { setup, create, getOne, setDelivered, getUserOrders, setupOrderItems, getOrderItems };
